...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, c):
        ...
        x = x + gate_mlp.unsqueeze(1) * self.mlp(   # 进行仿射变换后通过MLP,并乘以门控系数后残差连接
...
class MFDiT(nn.Module):
    ...
    def forward(self, x, t, r, y=None):
        ...
        """
        H, W = x.shape[-2:]
        # 将输入图像 x 通过 x_embedder 转换为嵌入表示，并加上位置编码  [b,t,d]=[48,256,384]
        ...
        x = self.unpatchify(x)                   # (N, out_channels, H, W) 还原为图像格式
        return x

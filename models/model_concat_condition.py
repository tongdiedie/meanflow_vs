...
# TODO: 完善 CrossAttentionBlock，用于 cross_attn 模式
class ConditionalConcatDiT(nn.Module):
    """
    条件DiT模型，用于虚拟染色
    ...
    """
    """
    策略一：Concatenation（在输入层拼接条件图像）
    - 直接复用你已有的 ConditionalMFDiT，只是把 cond_mode 固定为 'concat'。
    - 优点：实现最简单，稳定，显存占用中等；每一步采样都会通过 Transformer 反复利用条件信息。
    """
    def __init__(
        self,
        input_size=256,
